...
impl<I2C: embedded_hal_async::i2c::I2c> SGTL5000<I2C> {
    ...
    pub async fn get_chip_id(&mut self) -> Result<u8, I2C::Error> {
        let chip_id_and_revision = self.read_configuration(CHIP_ID).await?;
        Ok(chip_id_and_revision[0])
    }

    pub async fn get_chip_revision(&mut self) -> Result<u8, I2C::Error> {
        let chip_id_and_revision = self.read_configuration(CHIP_ID).await?;
    ...
    async fn init_clock_and_i2s(&mut self) -> Result<bool, I2C::Error> {
        // 44.1 kHz, 256*Fs
        self.update_configuration(CHIP_CLK_CTRL, 0x00, 0x04).await?;
        // SCLK=64*Fs, 16bit, I2S format
        self.update_configuration(CHIP_I2S_CTRL, 0x00, 0x30).await?;
        Ok(true)
    }
    ...
    async fn modify_configuration_bit(
        ...
        let mut analog_control_configuration = self.read_configuration(register).await.unwrap();
    ...
    /// volume 0 --> 127
    pub async fn headphone_volume(&mut self, value: u8) -> Result<bool, I2C::Error> {
    ...
    pub async fn select_adc_input(&mut self, input: AdcInputSources) -> Result<bool, I2C::Error> {
